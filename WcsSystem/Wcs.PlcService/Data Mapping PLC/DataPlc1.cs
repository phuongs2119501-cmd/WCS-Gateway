...
{
    public class DataPlc1
    {
        // ==========================================
        // DB500 PLC1 ADDRESS CONSTANTS
        // ==========================================

        // Commands (DB500, same address offset for both, but defined here for PLC1 context)
        ...
        // Shuttle 2 State (If read from PLC1 context/structure definition)
        ...
        // State Result
        public const string PLC1_DONE      = "DB500.DBX52.0";
        public const string PLC1_FAIL      = "DB500.DBX52.1";
        ...
        // Crane 2 Position (Read from PLC1)
        ...
        public string ReadBarcode(S7Connector connector)
        {
            ...
            var rawBytes = new byte[CHAR_COUNT];

            for (int i = 0; i < CHAR_COUNT; i++)
            {
                string address = $"DB{DB_NUMBER}.DBB{CHAR_START + i}";
                if (connector.TryReadChar(address, out char c))
                {
                    buffer[i] = c;
                    rawBytes[i] = (byte)c;
                }
